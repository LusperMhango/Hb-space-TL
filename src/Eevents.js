...
const Eevents = ({ date, title }) => (
  
  <div className="bg-white p-4 my-2 border rounded-md shadow-md">
    <div className="flex justify-between items-center mb-2">
      <div className="text-xs text-gray-500">{date}</div>
      <button className="bg-blue-200 text-blue-600 px-3 py-1 rounded-md">Register</button>
    </div>
    ...
    <div className="flex flex-col items-center p-4">
      ...
      <div className="bg-gray-200 w-full rounded-md">
        <div className="flex justify-between border-b">
          <button className="flex-1 py-2 border-r text-center">Upcoming</button>
          <button className="flex-1 py-2 text-center">Past</button>
        </div>
        <Eevents date="MON, JUN 17, 2024, 6:00 PM CAT" title="ARDUINO PROGRAMMING" />
        <Eevents date="TUE, JUN 18, 2024, 6:00 PM CAT" title="MICRO-CONTROLLERS" />
      </div>
    </div>
  </div>
);
